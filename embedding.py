# ==============================================================================
# 【模組說明】：embedding.py (本地端開源 BAAI/bge-base-zh-v1.5 向量嵌入與資料庫建置模組)
# 【主要功能】：讀取 'Chunks/' 資料夾中的 Chunk JSON 資料，使用本地端開源模型
#              BAAI/bge-base-zh-v1.5 (768 維向量) 將文字轉化為向量，並將向量與結構化中繼資料
#              儲存於 'Vector_Database/' 資料夾中建置本地向量知識庫。
# ==============================================================================
...
try:
    ...
    print("[embedding.py] [Error] 未找到 'sentence-transformers' 套件，請先在終端機執行 'pip install sentence-transformers'！")
...
def clean_directory(dir_path: str):
    ...
    for item in os.listdir(dir_path):
        ...
        try:
            ...
            print(f"[embedding.py] [Warning] 無法刪除舊檔案 '{item_path}': {e}")
...
def get_bge_model():
    ...
    if _MODEL_CACHE is None:
        ...
        print(f"[embedding.py] 正在載入本地開源向量模型 '{MODEL_NAME}' (免 API Key / 零 429 限流)...")
...
def build_embeddings_for_chunk_file(chunk_json_path: str, output_dir: str = None) -> bool:
    ...
    if not os.path.exists(chunk_json_path):
        print(f"[embedding.py] [Error] 找不到 Chunk JSON 檔: {chunk_json_path}")
        return False
    ...
    print(f"[embedding.py] 正在為 '{filename}' 建立本地 BAAI/bge-base-zh-v1.5 向量資料庫...")
    ...
    if not chunks:
        print(f"[embedding.py] '{filename}' 中沒有任何 Chunks。")
        return False

    # 取出待向量化的內容文字 (包含 Context Prefix)
    ...
    print(f"[embedding.py] [Success] 成功完成本地 768 維向量資料庫建置！")
...
def build_all_embeddings(chunks_dir: str = None, output_dir: str = None) -> bool:
    ...
    if not json_files:
        print(f"[embedding.py] 在 '{chunks_dir}' 目錄下找不到任何 Chunks JSON 檔案。")
        return False

    print(f"[embedding.py] 清理舊的 Vector_Database 資料夾內容...")
    clean_directory(output_dir)

    print(f"[embedding.py] 找到 {len(json_files)} 個 Chunk JSON 檔案，準備建置向量知識庫...")
