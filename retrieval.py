# ==============================================================================
# 【模組說明】：retrieval.py (向量檢索與相似度搜尋模組 - BAAI/bge-base-zh-v1.5 本地端)
# 【主要功能】：接收使用者輸入的問答檢索問題，使用本地端 BAAI/bge-base-zh-v1.5 模型將問題轉為 768 維查詢向量，
#              並與 'Vector_Database/' 資料夾中的向量進行餘弦相似度 (Cosine Similarity) 計算，
#              回傳最相關的前 Top-10 個 Chunk 檢索結果，並將檢索結果以標準 Markdown 格式
#              自動儲存於 'response/' 資料夾。
# ==============================================================================
...
try:
    ...
    print("[retrieval.py] [Error] 未找到 'sentence-transformers' 套件，請先在終端機執行 'pip install sentence-transformers'！")
...
def get_bge_retrieval_model():
    ...
    if _RETRIEVAL_MODEL_CACHE is None:
        ...
        print(f"[retrieval.py] 正在載入本地開源向量模型 '{MODEL_NAME}' (免 API Key / 零 429 限流)...")
...
def search_relevant_chunks(query: str, top_k: int = 10, filter_year: str = None) -> list[dict]:
    """
    【主要搜尋功能】：將查詢文字轉為向量，在 Vector_Database/ 中進行相似度比對並回傳 Top-K 個結果。
    """
    if not query.strip():
        print("[retrieval.py] 查詢字串不可為空。")
        return []

    if not os.path.exists(VECTOR_DB_DIR):
        print(f"[retrieval.py] [Error] 找不到向量資料庫目錄 '{VECTOR_DB_DIR}'，請先執行選單 [3] 建立向量資料庫！")
        return []

    npy_files = glob.glob(os.path.join(VECTOR_DB_DIR, "*_vectors.npy"))
    if not npy_files:
        print(f"[retrieval.py] [Error] 在 '{VECTOR_DB_DIR}' 目錄下未找到任何向量檔，請先執行選單 [3] 建立向量資料庫！")
        return []

    print(f"[retrieval.py] 正在使用本地端 BAAI/bge-base-zh-v1.5 模型為「{query}」生成向量並檢索...")
...
def save_retrieval_response_markdown(query: str, results: list[dict], output_dir: str = None) -> str:
    ...
    md_lines = []
    md_lines.append(f"# 財報問題檢索結果 (retrieval - BAAI/bge-base-zh-v1.5 本地模型) - Top {len(results)}")
    ...
    print(f"\n[retrieval.py] [Success] 檢索結果已成功寫入 Markdown 檔並儲存於：")
...
def interactive_retrieval_search():
    """選單 [5] 專用互動式查詢介面"""
    print("\n" + "=" * 74)
    print("        財報蒟蒻 RAG 系統 - 問題檢索 (retrieval - BAAI/bge-base-zh-v1.5 本地端)")
    print("=" * 74)
